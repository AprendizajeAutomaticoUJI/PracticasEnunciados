...
La función de pérdidas, en este caso, puede ser la siguiente:

```julia
perdidas(modelo, X, y) = Flux.Losses.mse(modelo(X), y)
```

Donde el primer argumento es el modelo, el segundo la matriz de variables predictoras y el tercera la el valor a predecir.
...
No olvides añadir celdas con comentarios. Todo el código que escribas debe estar razonado. Si no hay comentarios la práctica no estará apta.
